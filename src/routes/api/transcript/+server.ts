...
import { getTranscript } from '$lib/server/myed';
...
export const GET: RequestHandler = async ({ cookies }) => {
	...
	let key = sessionKey('transcript', session.cookies);
	...
	try {
		...
		key = sessionKey('transcript', session.cookies);
		try {
			const data = await getTranscript(session);
			setCache(key, data, 300);
			return json(data);
		} catch {
			return json({ error: 'Failed to load transcript' }, { status: 500 });
		}
	}
};
